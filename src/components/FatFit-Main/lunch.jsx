...
function Lunch({ username, onFoodChange }) {
  ...
  useEffect(() => {
    ...
    httpRequest(`${API_URL}/caloriecounter/${username}/lunch`, {
      credentials: "include"
    })
      .then(async (res) => {
  ...
  const handleDelete = async (foodName) => {
    if (!window.confirm(`Are you sure you want to delete '${foodName}' from lunch?`)) return;
    setLoading(true);
    setError(null);
    try {
      // Use the correct endpoint and method for your backend
      const res = await httpRequest(`${API_URL}/food/${username}/lunch`, {
        method: "DELETE",
  ...
  return (
    <div className="meal-box">
      <div className="meal-box-topline lunch-gradient" />
      ...
      {expanded && (
        <div className="meal-box-expanded">
          {loading && <div>Loading lunch foods...</div>}
          {error && <div style={{ color: "red" }}>{error}</div>}
          {foods.length === 0 && !loading && <div>No lunch foods for today.</div>}
      ...
      {showManage && (
        ...
        >
          ...
          >
            <h3>Manage Lunch Foods</h3>
            <ul style={{ listStyle: "none", padding: 0 }}>
              {foods.length === 0 && <li>No lunch foods to manage.</li>}
              {foods.map((food, idx) => (
